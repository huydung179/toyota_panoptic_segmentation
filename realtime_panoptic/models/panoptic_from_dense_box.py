...
from realtime_panoptic.utils.boxlist_ops import (
    boxlist_nms, cat_boxlist, remove_small_boxes)
...
class PanopticFromDenseBox:
    ...
    def process(
        ...
        for i, (l, o, b, c) in enumerate(zip(locations[:-1], box_cls, box_regression, centerness)):
            if self.is_training:
                layer_boxes = self.forward_for_single_feature_map(
                    l, o, b, c, image_sizes)
        ...
        bounding_box_feature_map = self.generate_box_feature_map(
            levelness_locations, box_regression, levelness_logits)

        # process semantic raw prediction
        semantic_logits = F.interpolate(
            semantic_logits, size=(h_map, w_map), mode='bilinear')
        semantic_logits = semantic_logits.view(
            N, c_semantic, h_map, w_map).permute(0, 2, 3, 1)
        ...
        if not self.is_training:
            for boxlist in boxlists:
                ...
                if len(masks.shape) == 3 and masks.shape[0] != 0:
                    masks = F.interpolate(masks.unsqueeze(0), size=(
                        h_map * 4, w_map * 4), mode='bilinear').squeeze()
                else:
    ...
    def generate_box_feature_map(self, location, box_regression, levelness_logits):
        ...
        for box_reg in box_regression:
            upscaled_box_reg.append(F.interpolate(
                box_reg, size=downsampled_shape, mode='bilinear').unsqueeze(1))
        ...
        box_feature_map = box_feature_map.view(
            N, 4, h_map, w_map).permute(0, 2, 3, 1)
    ...
    def mask_reconstruction(self, boxlists, box_feature_map, semantic_prob, box_feature_map_location, h_map, w_map):
        ...
        for i, (boxlist, per_image_bounding_box_feature_map, per_image_semantic_prob,
            ...
            if len(boxlist) > 0:
                ...
                per_image_cls_prob = per_image_semantic_prob[:,
                                                             propose_cls - 1].permute(1, 0)
                ...
                masks = bbox_correlation_map.view(
                    query_boxes.shape[0], h_map, w_map)
                if len(masks.shape) < 3:
                    masks = masks.unsqueeze(0)
                boxlist.add_field("mask", masks)
            else:
        ...
        for i in range(num_images):
            ...
            for j in range(1, self.num_classes):
                ...
                if len(inds) > 0:
                    ...
                    boxlist_for_class = BoxList(
                        boxes_j, boxlist.size, mode="xyxy")
