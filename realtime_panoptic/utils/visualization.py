...
def visualize_segmentation_image(predictions, original_image, colormap, fade_weight=0.5):
    ...
    colored_predictions_numpy = colored_predictions_numpy.reshape(
        original_image_height, original_image_width, 3)

    # Overlay images and predictions
    overlaid_predictions = original_image * fade_weight + \
        colored_predictions_numpy * (1 - fade_weight)
...
def random_color(base, max_dist=30):
    ...
    base = np.array(base)
    new_color = base + \
        np.random.randint(low=-max_dist, high=max_dist + 1, size=3)
...
def draw_mask(im, mask, alpha=0.5, color=None):
    ...
    """
    if color is None:
        color = DETECTRON_PALETTE[np.random.choice(
            len(DETECTRON_PALETTE))][::-1]
    color = np.asarray(color, dtype=np.int64)
    im = np.where(np.repeat(
        (mask > 0)[:, :, None], 3, axis=2), im * (1 - alpha) + color * alpha, im)
    im = im.astype('uint8')
...
def visualize_detection_image(predictions, original_image, label_id_to_names, fade_weight=0.8):
    ...
    for box, color, score, mask, label in zip(boxes, colors, scores, masks, label_names):
        ...
        cv2.putText(visualized_image, s, (x, y),
                    cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255), 1)

        top_left, bottom_right = box[:2].tolist(), box[2:].tolist()
        visualized_image = cv2.rectangle(visualized_image, tuple(
            top_left), tuple(bottom_right), tuple(color), 1)
    return visualized_image
