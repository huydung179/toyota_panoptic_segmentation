...
def demo():
    ...
    parser.add_argument("--input", metavar="FILE",
                        help="path to jpg/png file", required=True)
    ...
    # data pre-processing
    normalize_transform = P.Normalize(
    ...
    with torch.no_grad():
        ...
        instance_detection = [
            o.to('cpu') for o in panoptic_result["instance_segmentation_result"]]
        semseg_logics = [o.to('cpu')
                         for o in panoptic_result["semantic_segmentation_result"]]
        ...
        cv2.imwrite('semantic_segmentation_result.jpg', seg_vis.astype('uint8'))
        print(
            "Saved semantic segmentation visualization in semantic_segmentation_result.jpg")
        det_vis = visualize_detection_image(
            instance_detection[0], input_image, cityscapes_instance_label_name)
        cv2.imwrite('instance_segmentation_result.jpg', det_vis.astype('uint8'))
        print(
